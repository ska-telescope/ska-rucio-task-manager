...
if [ "${RUCIO_CFG_CLIENT_AUTH_TYPE,,}" == 'userpass' ]
then
  if [ -v RUCIO_CFG_CLIENT_USERNAME ] && [ -v RUCIO_CFG_CLIENT_PASSWORD ]
  then
    echo "proceeding with userpass authentication..."
  else
    echo "requested userpass auth but one or more of \$RUCIO_CFG_CLIENT_USERNAME or \$RUCIO_CFG_CLIENT_PASSWORD are not set"
    echo "quitting"
    exit
  fi
elif [ "${RUCIO_CFG_CLIENT_AUTH_TYPE,,}" == 'oidc' ]
then
  ...
  then
    ...
    cp "/tmp/access_token" "/tmp/tmp_auth_token_for_account_$RUCIO_CFG_CLIENT_ACCOUNT"
    export BEARER_TOKEN=`cat /tmp/tmp_auth_token_for_account_$RUCIO_CFG_CLIENT_ACCOUNT`
  elif [ -v OIDC_CLIENT_ID ] && [ -v OIDC_CLIENT_SECRET ] && [ -v OIDC_TOKEN_ENDPOINT ] && [ -v RUCIO_CFG_CLIENT_OIDC_SCOPE ] && \
    [ -v RUCIO_CFG_CLIENT_OIDC_AUDIENCE ] && [ -v RUCIO_CFG_CLIENT_ACCOUNT ]                                                            # if IAM client's credentials have been passed in (i.e. using a service client w/ client_credentials flow)
  then
    echo "proceeding with oidc authentication via passed client credentials..."
    curl -s -u "$OIDC_CLIENT_ID:$OIDC_CLIENT_SECRET" -d grant_type=client_credentials -d scope="$RUCIO_CFG_CLIENT_OIDC_SCOPE" \
      -d audience="$RUCIO_CFG_CLIENT_OIDC_AUDIENCE" $OIDC_TOKEN_ENDPOINT \
      | jq -j '.access_token' > "/tmp/tmp_auth_token_for_account_$RUCIO_CFG_CLIENT_ACCOUNT"
    export BEARER_TOKEN=`cat /tmp/tmp_auth_token_for_account_$RUCIO_CFG_CLIENT_ACCOUNT`
  elif [ -v OIDC_AGENT_AUTH_CLIENT_CFG_VALUE ] && [ -v OIDC_AGENT_AUTH_CLIENT_CFG_PASSWORD ] && [ -v RUCIO_CFG_CLIENT_ACCOUNT ]         # if oidc-agent config is being passed in as a value (e.g. from a k8s secret)
  then
    ...
    export BEARER_TOKEN=`oidc-token --scope "$RUCIO_CFG_CLIENT_OIDC_SCOPE" --aud "$RUCIO_CFG_CLIENT_OIDC_AUDIENCE" $OIDC_CLIENT_NAME`
    echo $BEARER_TOKEN > "/tmp/tmp_auth_token_for_account_$RUCIO_CFG_CLIENT_ACCOUNT"
  elif [ -v OIDC_ACCESS_TOKEN ] && [ -v RUCIO_CFG_CLIENT_ACCOUNT ]                                                                      # if access token is being passed in directly
  then
    echo "proceeding with oidc authentication using an access token..."
    echo "$OIDC_ACCESS_TOKEN" > "/tmp/tmp_auth_token_for_account_$RUCIO_CFG_CLIENT_ACCOUNT"
  ...
  tr -d '\n' < "/tmp/tmp_auth_token_for_account_$RUCIO_CFG_CLIENT_ACCOUNT" > "/tmp/auth_token_for_account_$RUCIO_CFG_CLIENT_ACCOUNT"
  # move this token to the location expected by Rucio
  mkdir -p /tmp/user/.rucio_user/
  mv "/tmp/auth_token_for_account_$RUCIO_CFG_CLIENT_ACCOUNT" /tmp/user/.rucio_user/
fi
