...
class TestIngestionEphemeral(Task):
    ...
    def start_ingest_service(self):
        ...
        and the processes it pools) can be signaled on teardown.
...
class TestIngestionEphemeralSingleSweep(Task):
    """ Test notification-based ingestion by running an ephemeral instance of the
    ska-src-dm-di-ingestor service for a single ingestion sweep.

    This follows the same test flow as TestIngestionEphemeral, but uses the current
    (ingestion notification file based) version of the ingestor service. Rather than leaving
    the service polling a staging area in the background for the duration of the test, the data
    files and the ingest notification file are supplied to the staging area first and the
    service is then started with --run-once so that it performs a single ingestion sweep over
    the configured ingest states. The service - along with its base directory - is torn down
    once the test has finished.
    """

    # Names of the subdirectories of the service base directory corresponding to the ingest
    # states used by the test, i.e. the lowercased names of the ingestor's IngestStates.
    staging_subdir = "staging"
    processed_metadata_subdir = "processed_metadata"
    failed_subdirs = (
        "failed_ingest_request",
        "failed_processing_data",
        "failed_processing_metadata",
    )

    # The ingest states to sweep (in order) and the pipelines to use per state. These mirror
    # the current deployment of the ska-src-dm-di-ingestor service; a single sweep over these
    # states carries a notification through the full state machine.
    default_ingestion_states = "STAGING,READY_FOR_INGEST,PROCESSED_DATA"
    default_ingestion_pipelines_fqn = ",".join([
        "ingestor.pipelines.post_staging.LinkedFromIngestionNotificationV1PostStagingPipeline",
        "ingestor.pipelines.post_ready_for_ingest.LinkedFromIngestionNotificationV1PostReadyForIngestPipeline",
        "ingestor.pipelines.post_processed_data.LinkedFromIngestionNotificationV1PostProcessedDataPipeline",
    ])
    ...
    def __init__(self, logger):
        ...
        - ingestion_states: The ingest states (comma separated, in order) the single sweep
            should act upon.
        - ingestion_pipelines_fqn: The FQNs (comma separated) of the pipelines to use.
        - ingestion_polling_frequency_s: The interval the ingestion service sleeps for after
            sweeping each state.
        - ingestion_iteration_batch_size: The number of work items that the ingestion service
            should batch together for ingestion per iteration.
        - ingestion_n_processes: The number of processes the ingestion service should pool.
        - data_ingestion_backend_name: The name of the data ingestion backend to use.
        - metadata_ingestion_backend_name: The name of the metadata ingestion backend to use.
        - rucio_ingest_rse_name: The (Rucio) identifier of the RSE to ingest data into.
        - rucio_pfn_basepath: The PFN basepath (required for non-deterministic data ingestion
            backends only).
        - keep_base_dir: Retain the service base directory after the test (useful for
            debugging).
        - n_retries: The number of times to poll for the sweep to process the notification.
        - delay_s: The interval at which to poll at in seconds.
        ...
        self.ingestion_pipelines_fqn = None
        self.ingestion_polling_frequency_s = None
        self.ingestion_iteration_batch_size = None
        self.ingestion_n_processes = None
        self.data_ingestion_backend_name = None
        self.metadata_ingestion_backend_name = None
        self.rucio_ingest_rse_name = None
        self.rucio_pfn_basepath = None
        self.keep_base_dir = None
        self.n_retries = None
    ...
    def start_ingest_service(self):
        """ Start an ephemeral instance of the ingestion service as a child process to perform a
        single ingestion sweep.

        The service is started with --run-once so that each of the configured ingest states is
        swept exactly once. It is started in its own session so that the whole process group (the
        service and the processes it pools) can be signaled on teardown.
        ...
        cmd = [self.ingest_cli,
               ...
               '--frequency', str(self.ingestion_polling_frequency_s),
               '--batch-size', str(self.ingestion_iteration_batch_size),
               '--notification-file-suffix', self.notification_file_suffix,
               '--pipelines-fqn', self.ingestion_pipelines_fqn,
               '--n-processes', str(self.ingestion_n_processes),
               '--data-ingestion-backend-name', self.data_ingestion_backend_name,
               '--metadata-ingestion-backend-name', self.metadata_ingestion_backend_name,
               '--rucio-ingest-rse-name', self.rucio_ingest_rse_name]
        if self.rucio_pfn_basepath:
            cmd = cmd + ['--rucio-pfn-basepath', self.rucio_pfn_basepath]

        # The service output is redirected to a log file in the base directory rather than being
        # interleaved with the task output.
        self.service_log_path = os.path.join(self.ingest_dir, "service.log")
        self.logger.info("Starting single sweep of the ingestion service: {}".format(" ".join(cmd)))
        self.logger.info("Ingestion service log: {}".format(self.service_log_path))
        try:
            service_log = open(self.service_log_path, 'w')
            self.service_process = subprocess.Popen(
    ...
    def wait_for_ingest_sweep(self, notification_name):
        """ Wait for the single ingestion sweep to fully process the supplied notification.

        The notification has been fully processed once it reaches the processed_metadata state.
        If the notification lands in one of the failed states, or the service exits before the
        notification has been processed, the sweep is considered to have failed.

        :param str notification_name: The name of the supplied notification file.
        :return: True if the notification was fully processed, else False.
        """
        max_retries = self.n_retries
        processed_dir = os.path.join(self.ingest_dir, self.processed_metadata_subdir)
        retries = 0
        while retries < max_retries:
            if list(Path(processed_dir).rglob(notification_name)):
                self.logger.info("Notification processed: {}".format(notification_name))
                return True
            for failed_subdir in self.failed_subdirs:
                failed_dir = os.path.join(self.ingest_dir, failed_subdir)
                if list(Path(failed_dir).rglob(notification_name)):
                    self.logger.critical(
                        "Notification {} moved to {} during the sweep".format(
                            notification_name,
                            failed_subdir
                        )
                    )
                    ...
                    return False
            if self.service_process.poll() is not None:
                # The service performs a single sweep only, so it exiting without the
                # notification having reached a terminal state means the ingest failed.
                self.logger.critical(
                    "Ingestion service exited with code {} before notification {} was "
                    "processed".format(self.service_process.returncode, notification_name)
                )
                self.log_ingest_service_output()
                return False
            self.logger.info(
                "Waiting for the ingestion sweep to process notification {}...".format(
                    notification_name)
            )
            time.sleep(self.delay_s)
            retries += 1
    ...
    def run(self, args, kwargs):
        ...
        try:
            ...
            self.ingestion_pipelines_fqn = kwargs.get(
                "ingestion_pipelines_fqn", self.default_ingestion_pipelines_fqn)
            self.ingestion_polling_frequency_s = kwargs.get("ingestion_polling_frequency_s", 1)
            self.ingestion_iteration_batch_size = kwargs.get("ingestion_iteration_batch_size", 50)
            self.ingestion_n_processes = kwargs.get("ingestion_n_processes", 1)
            self.data_ingestion_backend_name = kwargs.get("data_ingestion_backend_name", "rucio")
            self.metadata_ingestion_backend_name = kwargs.get("metadata_ingestion_backend_name", "rucio")
            self.rucio_ingest_rse_name = kwargs["rucio_ingest_rse_name"]
            self.rucio_pfn_basepath = kwargs.get("rucio_pfn_basepath")
            self.keep_base_dir = kwargs.get("keep_base_dir", False)
            self.n_retries = kwargs["n_retries"]
        ...
        if self.ingest_dir:
            ...
            self.ingest_dir = tempfile.mkdtemp(prefix="ingest-single-sweep-")
            self.is_base_dir_temporary = True
    ...
    def run_test(self):
        """ Stage the files and the notification, run a single ingestion sweep over them and
        check the expected DIDs have been registered. """
        # Set up identifiers for this test run:
        eb_id = "eb-{}".format(uuid.uuid4().hex[:8])
        sbd_id = "sbd-{}".format(uuid.uuid4().hex[:8])

        # Set up log message:
        test_id = "ingestion_single_sweep_test_{}".format(datetime.now().isoformat())
        entry = {
            "task_name": self.task_name,
            "name": test_id,
            "scope": self.scope,
        ...
        # Generate FITS files of (approximately) specified sizes in a unique source subdirectory
        # of the service base directory:
        run_source_dir = os.path.join(self.ingest_dir, "source", eb_id)
        os.makedirs(run_source_dir, exist_ok=True)
        target_ra, target_dec = random.uniform(0, 360), random.uniform(-60, 60)
        for idx in range(self.n_files):
            file_path = os.path.join(
        ...
        # staging_base_url is set to the location the files will have once supplied to the ingestor's
        # staging area so access_urls in the notification resolve correctly:
        staging_dir = os.path.join(self.ingest_dir, self.staging_subdir, self.scope)
        builder = (
            NotificationBuilder()
            .scan_directory(run_source_dir, staging_base_url="file://{}".format(staging_dir))
            .set_project_info(
        ...
        # Supply both the data files and the notification file to the staging area before the
        # service is started, so everything is picked up by the single sweep. The notification
        # file is still copied last for consistency with the other ingestion tests:
        os.makedirs(staging_dir, exist_ok=True)
        for file_name in sorted(os.listdir(run_source_dir)):
            if file_name == notification_name:
                continue
        ...
        # Start the ephemeral service to perform a single ingestion sweep over the staged files:
        if not self.start_ingest_service():
            return False

        # Poll (every <delay_s> sec) for the sweep to fully process the notification (i.e. for
        # it to appear in the processed_metadata area):
        processed = self.wait_for_ingest_sweep(notification_name)

        # The sweep has finished, so the expected file DIDs should have been added by the
        # ingestion service (no further sweeps will run):
        did_client = DIDClient()
        succeeded = 0
        failed = 0
        for did in file_dids:
            ...
            try:
                ...
                self.logger.critical(
                    "DID {} not found after the single ingestion sweep".format(did_name)
                )
                failed += 1
            except Exception as e:
                self.logger.critical(
